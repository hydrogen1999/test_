...
class SpectralAnnealing:
    def __init__(self, instance_name: str, dataset: str, random_seed: int = None,
                 ...
                 disable_warm_start: bool = False, precision: int = 32, **kwargs):
        ...
        if self.alpha is None:
            ...
            for alpha_idx, alpha in enumerate(alpha_list):
                try:
                    ...
                    # Disable warm start if flag is set
                    ...
                    # === Compute eigenvalue and spectral energy bound ===
                    lambda1 = float(evals[0])  # principal eigenvalue

                    # degree_scale = sum_i d_i^alpha  (already normalized degrees)
                    degree_scale = float(cp.sum(D_vec))

                    # correct desnormalization factor for the spectral bound:
                    # Bound_normalized   >= -0.5 * lambda1 * degree_scale
                    # Bound_real_units   >= -0.5 * lambda1 * degree_scale * normalization_factor
                    normalization_factor = float(self._normalization_factor)
                    # Correct spectral lower bound in real units:
                ...
                # print(f"GPU memory used  (alpha={alpha}): {cp.cuda.runtime.memGetInfo()[0] / 1e9:.2f} / {cp.cuda.runtime.memGetInfo()[1] / 1e9:.2f} GB")
            ...
            # Add memory monitoring
            # print(f"GPU memory used  (local search): {cp.cuda.runtime.memGetInfo()[0] / 1e9:.2f} / {cp.cuda.runtime.memGetInfo()[1] / 1e9:.2f} GB")
    ...
    def _get_alpha_values(self, mode: int) -> list:    
        if mode == 0:
            ...
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 1))]
        elif mode == 2:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 2))]
        elif mode == 3:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 4))]
        elif mode == 4:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 8))]
        elif mode == 5:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 16))]
        elif mode == 6:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 32))]
        elif mode == 7:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 64))]
        elif mode == 8:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 128))]
        elif mode == 9:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 256))]
        elif mode == 10:
            return [float(x) for x in cp.sqrt(cp.linspace(0, 1, 512))]
        else:
    ...
    def _solve_eigenvalue_problem(self, J_csr, D_vec, beta, k=1, v0=None):  
        ...
        # Ensure iter_count is a Python int (not CuPy array)
        ...
        return evals[:k], X[:, :k], evecs_y[:, :k], iter_count
