...
class SpectralAnnealing:
    def __init__(self, instance_name: str, dataset: str, random_seed: int = None,
                 ...
                 eigsh_tol: float = 1e-6, eigsh_maxiter: int = 1000, eigsh_ncv: int = None,
                 run_name: str = None, disable_warm_start: bool = False, precision: int = 32, **kwargs):
        ...
        self.eigsh_ncv = int(eigsh_ncv) if eigsh_ncv is not None else None
        ...
        print(f"  eigsh_ncv: {self.eigsh_ncv}")
    ...
    def _solve_eigenvalue_problem(self, J_csr, D_vec, beta, k=1, v0=None):  
        ...
        if self.eigsh_ncv is not None:
            eigsh_kwargs['ncv'] = self.eigsh_ncv
        
        evals, evecs_y, iter_count = cpx_splinalg.eigsh(
            self._S_op, k=k, which="SA", **eigsh_kwargs
        )
