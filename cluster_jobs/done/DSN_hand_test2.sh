#!/bin/bash
#SBATCH --job-name=DSN_hand_test2
...
#SBATCH --output=cluster_logs/DSN_hand_test2_%j.out
#SBATCH --error=cluster_logs/DSN_hand_test2_%j.err
...
if [ $? -eq 0 ]; then
  echo "✓ DSN on hand_test2 SUCCESS"
  TOTAL=$((TOTAL + 1))
else
  echo "✗ DSN on hand_test2 FAILED"
  exit 1
fi
